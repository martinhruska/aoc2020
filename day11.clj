(use 'clojure.data)
(def input "LLLLL.LLL.LLLLLLLLLLLLLLL.LLLLLLLLL.LLLLLLLLLLLLL.LLLLLLL.LLLLLLL.LLLLL.LLLLLLLLLLLLLLLLLLL.LLLLL
...
(def processed (vec (map clojure.string/trim (clojure.string/split-lines input))))
(def processed1 (vec (map clojure.string/trim (clojure.string/split-lines input1))))

(defn parse-line [line-num act-line]
    (loop [j 0 line act-line res {}]
        (if (= (count line) 0)
            res
            (recur (inc j) (rest line) (assoc res (list line-num j) (first line))))))

(defn parse [input]
    (loop [i 0 lines input res {}]
        (if (= (count lines) 0)
            res
            (recur (inc i) (rest lines) (merge res (parse-line i (first lines))
                )))))

(defn get-all-combs [i j]
    (for [x (range 0 (dec i)) y (range 0 (dec j))] (list x y)))

(def neigh
    (for [x (range -1 2) y (range -1 2) :when (not (and (= x 0 ) (= x y)))] (list x y)))

(defn get-new [act neigh]
(let [n (count (filter #(= \# %) neigh))]
    (cond
        (and (= act \L) (zero? n)) \#
        (and (= act \#) (>= n 5)) \L
        ;(and (= act \#) (>= n 4)) \L
...
(defn what-see [cord dir mp]
(let [next-cord (list (+ (first cord) (first dir)) (+ (second cord) (second dir)))
      act (get mp next-cord \L)]
    (if (= act \.)
        (what-see next-cord dir mp)
        act)))

(defn get-neigh [cord mp]
(let [x (first cord)
      y (second cord)]
    (map #(get mp (list (+ x (first %)) (+ y (second %))) \L) neigh)))

(defn get-neigh2 [cord mp]
    (map #(what-see cord % mp) neigh))

(defn one-round [mp]
    (loop [act mp nxt {}]
        (let [cords (first (first act))
              val (second (first act))]
            (if (= (count act) 0)
                nxt
                (recur (rest act) (assoc nxt cords (get-new val (get-neigh2 cords mp))))))))

(defn iter [mp]
    (loop [old mp]
        (let [new (one-round old)
            _ (println "ROUND\n") ;(str "\n" old "\n" new))
            ]
            (if (= old new)
                new
...
(defn count-occ [mp]
    (count (for [x mp :when (= (second x) \#)] x)))
