(def input "mirrored beige bags contain 1 drab brown bag, 3 dotted crimson bags.
...
(def split-input (map clojure.string/trim (clojure.string/split-lines input)))
(def split-input1 (map clojure.string/trim (clojure.string/split-lines input1)))

(def line "wavy yellow bags contain 5 vibrant fuchsia bags, 3 drab fuchsia bags.")

(defn string-to-pair [word]
(let [splitted (clojure.string/split word #" ")]
    (if (clojure.string/includes? word "no other")
        (list )
        (list (Integer/parseInt (first splitted)) (clojure.string/join " " (rest splitted))))))

(defn parse-line [line]
(let [contain-split (clojure.string/split (clojure.string/replace line #"bag[s]?\.?" "") #"contain")
      string-bags (map clojure.string/trim (flatten [(first contain-split) (clojure.string/split (second contain-split) #",")]))]
    (conj (map string-to-pair (rest string-bags)) (first string-bags))))

(defn create-dep-graph [input]
    (for [item input]
        (flatten (list (first item) (map #(if (= (count %1) 0) (list ) (second %1)) (rest item))))))

(defn compute-deps [graph act]
    (set (for [item graph
        :when (some #(= act %) (rest item))]
        (first item))))

(defn subtr [new all]
    (for [item new
        :when (not (contains? all item))]
        item))

(defn compute-set [graph todo all computer]
(let [act (first todo)
      new-color (computer graph act)
      not-processed (subtr new-color all)]
    (if (= (count todo) 0)
        all
        (compute-set graph (flatten (conj (rest todo) not-processed)) (clojure.set/union all new-color) computer))))

(defn build-tree [graph name]
    (first (for [item graph
        :when (= (first item) name)]
        item)))

(defn expand [graph l]
    (map #(list (first %1) (build-tree graph (second %1))) l))

(defn build-tree1 [graph name]
(let [dep (build-tree graph name)]
    (if (= (count (second dep)) 0)
        (list )
        (for [i (rest dep)] (list (first i) (build-tree1 graph (second i)))))))

(defn evalu [graph]
(let [_ (println "HERE " graph (= (list ) graph) (first graph) (type (first graph)))])
    (cond
        (= (list ) graph) 0
        (int? (first graph)) (+ (first graph) (* (first graph) (evalu (rest graph))))
        ; (list? (first graph)) (reduce + (map evalu graph))
        ;:else (assert false)))
        :else (reduce + (map evalu graph))))

(def parsed-input (map parse-line split-input))
(def parsed-input1 (map parse-line split-input1))
(def dep-graph (create-dep-graph parsed-input))
(count (compute-set dep-graph ["shiny gold"] #{} compute-deps))