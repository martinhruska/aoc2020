...
(defn parseIntg [x] (Integer/parseInt x))
(defn parse-interval [interval]
    (map parseIntg (clojure.string/split interval #"-")))


(defn parse-line [line]
    (let [splitted (clojure.string/split line #":")
          lhs-split (clojure.string/split (first splitted) #" ")
          rhs-split (clojure.string/trim (second splitted))
          check-letter (second lhs-split)
          interval (parse-interval (first lhs-split))]
      (vector check-letter interval rhs-split)))

(defn cond1 [letter interval password]
    (let [count (reduce #(if (= letter (str %2)) (+ %1 1) %1) 0 password)]
      (and (<= (first interval) count) (<= count (second interval))))) 

(defn cond2 [letter interval password]
    (let [count (reduce #(if (= letter (str %2)) (+ %1 1) %1) 0 password)
           l1 (str (nth password (- (first interval) 1)))
           l2 (str (nth password (- (second interval) 1)))]
      (and (or (= l1 letter) (= l2 letter)) (not= l1 l2)))) 

(defn right? [line check-cond]
    (let [pr (parse-line line)]
    (if (check-cond (nth pr 0) (nth pr 1) (nth pr 2)) true false)))

(defn process [sum line]
    (if (right? (clojure.string/trim line) cond2) (+ sum 1) sum))

(reduce process 0 (clojure.string/split-lines input))