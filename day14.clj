...
(def input "mask = 1101000X0110100X1X1X00001010XX00X0X0
    mem[48482] = 6450058
    mem[5309] = 2219920
    mem[27274] = 43286042
    mem[40233] = 504849
...
(def process (map clojure.string/trim (clojure.string/split-lines input)))
(def process1 (map clojure.string/trim (clojure.string/split-lines input1)))
(def process2 (map clojure.string/trim (clojure.string/split-lines input2)))

(defn toBinary [num]
    (let [binary (Integer/toBinaryString num)]
        (apply str (apply str (repeat (- 36 (count binary)) "0"))  binary)))

(defn applyMask [val mask]
    (let [dbls (map vector mask val)
          res (map #(if (= \X (first %1)) (second %) (first %))  dbls)]
            (apply str res)))

(defn findNum [lhs]
    (Integer/parseInt (re-find #"\d+" lhs)))

(defn toDec [bin]
    (Long/parseLong bin 2))

(defn solve [in]
    (loop [rst in mask "" res {}]
    (let [actLine (if (= (count rst) 0) (list "" "") (map clojure.string/trim (clojure.string/split (first rst) #"=")))
          lhs (first actLine)
          rhs (second actLine)
        _ (println lhs rhs rst mask)]
        (cond
            (= 0 (count rst)) res
            (= "mask" lhs) (recur (rest rst) rhs res)
            :else (recur (rest rst) mask (assoc res (findNum lhs) (applyMask (toBinary (Integer/parseInt rhs)) mask)))))))

(defn comp [res]
    (reduce + 0 (map toDec (vals res))))
...
(defn applyMask1 [val mask]
    (let [dbls (map vector mask val)
          res (map #(if (or (= \1 (first %1)) (= \X (first %1))) (first %) (second %))  dbls)]
            (apply str res)))

(defn addrComb [mask comb]
    (loop [i 0 j 0 res ""]
        (if (>= i (count mask))
            res
            (let [c (nth mask i)
              isX (= c \X)
              nextChar (if isX (nth comb j) (nth mask i))]
                (recur (inc i) (if isX (inc j) j) (str res nextChar))))))

(defn addressesFromMask [maskedAddr]
    (let [xCount (count (re-seq #"X" maskedAddr))
          combs (apply cartesian-product (repeat xCount (list 0 1)))]
        (map #(addrComb maskedAddr (apply str %1)) combs)))

(defn assignToAddr [addrs val mem]
    (loop [a addrs res mem]
        (if (= (count a) 0)
            res
            (recur (rest a) (assoc res (toDec (first a)) val)))))
...
(defn solve1 [in]
    ...
    (let [actLine (if (= (count rst) 0) (list "" "") (map clojure.string/trim (clojure.string/split (first rst) #"=")))
          lhs (first actLine)
          rhs (second actLine)
        _ (println lhs rhs mask)]
        (cond
            (= 0 (count rst)) res
            (= "mask" lhs) (recur (rest rst) rhs res)
            :else (let [maskedAddr (applyMask1 (toBinary (findNum lhs)) mask)]
                (recur (rest rst) mask (assignToAddr (addressesFromMask maskedAddr) (Integer/parseInt rhs) res)))))))