...
(def in (map #(clojure.string/split %1 #"\ +") processed-in))

(defn add-word-to-set [word set]
    (map #(conj set %1) word))

(defn unique-letters [word]
    (count (keys (frequencies word))))

(defn compute-group [group]
    (unique-letters (clojure.string/join "" group)))
(defn compute-group2 [group]
    (count (apply clojure.set/intersection (map #(set %1) group))))

(defn compute-groups [groups]
    (reduce + (map compute-group2 groups)))
(map println in)
()