(def input-test "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
(def input "eyr:2024 pid:662406624 hcl:#cfa07d byr:1947 iyr:2015 ecl:amb hgt:150cm
...
; (def processed-in (map #(clojure.string/trim(clojure.string/replace %1 #"\p{C}" "")) (clojure.string/split input #" \n")))
(def processed-in (map #(clojure.string/trim(clojure.string/replace %1 #"\p{C}" "")) (clojure.string/split (clojure.string/trim input) #"\n\ *\n")))

(defn colon-count [line] (let [_ (println line)] (- (count (clojure.string/split line #":")) 1)))

(defn split-line [line] (clojure.string/split line #"\ +"))

(defn check-interval [num low high] (and (<= low num) (<= num high)))

(defn check-height [value]
    (let [pure (reduce str (filter (fn [x] (Character/isDigit x)) value))]
        (cond
            (clojure.string/includes? value "cm") (check-interval (Integer/parseInt pure) 150 193) 
            (clojure.string/includes? value "in") (check-interval (Integer/parseInt pure) 59 76)
            :else false)))

(defn check-item [item]
    (let [pair (clojure.string/split item #":")
          key (first pair)
          value (second pair)
          _ (println value pair)]
          (cond 
            (= key "byr") (check-interval (Integer/parseInt value) 1920 2002)
            (= key "iyr") (check-interval (Integer/parseInt value) 2010 2020)
            (= key "eyr") (check-interval (Integer/parseInt value) 2020 2030)
            (= key "hgt") (check-height value)
            (= key "hcl") (not= (re-matches #"#\w\w\w\w\w\w" value) nil)
            (= key "ecl") (and (contains? #{"amb" "blu" "brn" "gry" "grn" "hzl" "oth"} value)) 
            (= key "pid") (not= (re-matches #"\d\d\d\d\d\d\d\d\d" value) nil)
            (= key "cid") true)))

(defn check-items [items]
    (reduce #(and %1 (check-item %2)) true items))

(defn comp [] (filter #(or (= 8 (colon-count %1)) (and (= 7 (colon-count %1)) (not (clojure.string/includes? %1 "cid")))) processed-in))

(defn check-by-item [] (filter check-items (map split-line (comp))))